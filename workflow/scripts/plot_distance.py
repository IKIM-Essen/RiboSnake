...
while i < len(directories):
    ...
    if name in directories[i]:
        df = pd.read_csv(str(snakemake.input)+ "/" + directories[i]+"/data"+"/distance-matrix.tsv", sep = "\t", header = 0, index_col=0)
        matrix = np.triu(df)
        fig, ax = plt.subplots(figsize=(20, 15))
        heatmap = sb.heatmap(df, mask = matrix, cbar_kws={'label': 'Distance'}, annot=True, fmt=".1f")
